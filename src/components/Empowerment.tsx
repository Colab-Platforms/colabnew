...
const Card = ({ card, index }: { card: typeof cards[0], index: number }) => {
    ...
    return (
        ...
        >
            ...
            <div className={`absolute inset-0 bg-black/60 transition-opacity duration-500 ${isHovered ? 'opacity-100' : 'opacity-0'}`} />
            ...
            <div className={`absolute left-4 top-1/2 -translate-y-1/2 w-12 h-3/4 flex items-center justify-center z-20 pointer-events-none transition-opacity duration-300 ${isHovered ? 'opacity-0' : 'opacity-100'}`}>
                <span className="text-white font-['AloverDisplay'] text-xl tracking-widest uppercase -rotate-90 whitespace-nowrap">
                    {card.label}
                </span>
            </div>

            {/* Content (Visible on Hover) */}
            <div className={`absolute inset-0 flex flex-col justify-center items-center p-8 z-30 text-center transition-all duration-500 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <h3 className="text-3xl font-bold text-white mb-4 font-['AloverDisplay']">
                    {card.title}
                </h3>
                <p className="text-gray-200 text-lg leading-relaxed font-['AloverDisplay'] max-w-md">
                    {card.description}
                </p>
