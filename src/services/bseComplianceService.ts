...
export const fetchBSECompliance = async (
  ...
  try {
    ...
    const url = '/api/bse-compliance';
    
    console.log('🔄 Fetching BSE compliance from Vercel API...');
    ...
    if (data.Table && Array.isArray(data.Table)) {
      const documents: ComplianceDocument[] = data.Table.map((item: any, index: number) => ({
        id: `bse-${index}`,
      ...
      console.log('✅ Real BSE compliance data fetched:', documents.length);
      return documents;
    }
